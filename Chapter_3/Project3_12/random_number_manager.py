...
class RandomNumberManager:
    def __init__(self, N, p, nb_trials):
        ...
        self.poisson_mean = self.p * self.nb_numbers
        self.poisson_var = self.poisson_mean
        self.poisson_sd = sqrt(self.poisson_var)
        self.poisson_histogram = self.nb_trials * np.array([self.poisson(s) for s in range(0, N + 1)])
    ...
    def print_results1(self):
        # Calculate deviations from theory
        print("Comparison between theoretical and simulated results.")
    ...
    def print_results2(self):
        # Calculate deviations from theory
        print("Comparison between theoretical and Poisson results.")
        mean_dev = round(100 * abs(self.poisson_mean - self.theoretical_mean) / self.theoretical_mean, 3)
        var_dev = round(100 * abs(self.poisson_var - self.theoretical_var) / self.theoretical_var, 3)
        sd_dev = round(100 * abs(self.poisson_sd - self.theoretical_sd) / self.theoretical_sd, 3)
        # Round theoretical and Poisson results to 3 decimals
        self.poisson_mean = round(self.poisson_mean, 3)
        self.poisson_var = round(self.poisson_var, 3)
        self.poisson_sd = round(self.poisson_sd, 3)
        self.theoretical_mean = round(self.theoretical_mean, 3)
        self.theoretical_var = round(self.theoretical_var, 3)
        self.theoretical_sd = round(self.theoretical_sd, 3)

        x = PrettyTable()
        x.field_names = ["Theoretical mean", "Poisson mean", "Mean dev (%)"]
        x.add_row([self.theoretical_mean, self.poisson_mean, mean_dev])
        print(x)

        y = PrettyTable()
        y.field_names = ["Theoretical var", "Poisson var", "Var dev (%)"]
        y.add_row([self.theoretical_var, self.poisson_var, var_dev])
        print(y)

        z = PrettyTable()
        z.field_names = ["Theoretical SD", "Poisson SD", "SD dev (%)"]
        z.add_row([self.theoretical_sd, self.poisson_sd, sd_dev])
        print(z)
    ...
    def plot_histograms(self):
        ...
        plt.scatter(self.S, self.poisson_histogram, label='Poisson histogram')
        plt.title(f'N = {self.nb_numbers}, mu = {self.poisson_mean}, p = {self.p}. {self.nb_trials} trials.')
        ...
        deviations3 = np.abs((self.theoretical_histogram - self.poisson_histogram))
        ...
        plt.scatter(self.S, deviations3, label='Deviations of Poisson approximation from theory')
        plt.xlabel("s")
        plt.ylabel("Error")
        plt.legend(loc='upper right')
    ...
    def poisson(self, s):
        result = exp(-self.poisson_mean)
        for i in range(s):
            result *= (self.poisson_mean/(i+1))
        return result